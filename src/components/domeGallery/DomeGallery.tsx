...
type ImageItem = string | { image_url: string; alt?: string };
...
const DEFAULT_IMAGES: PhotoDetails[] = [
  {
    image_url:
    ...
    uploaded_by_name: "Abstract art",
    _id: "1",
    event_id: "1",
    uploaded_by_user_id: "1",

    filename: "abstract1.png",
    original_filename: "abstract1.png",
  ...
  },
  {
    ...
    filename: "abstract2.png",
    original_filename: "abstract2.png",
    uploaded_at: "2024-01-01T00:00:00Z",
  },
];
...
function buildItems(pool: PhotoDetails[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map((c) => ({ ...c, image_url: "", alt: "" }));
  }
  ...
  const normalizedImages = pool.map((image) => {
    if (typeof image === "string") {
      return { image_url: image, alt: "" };
    }
    return { image_url: image.image_url || "", alt: image.uploaded_by_name || "" };
  });
  ...
  for (let i = 1; i < usedImages.length; i++) {
    if (usedImages[i].image_url === usedImages[i - 1].image_url) {
      for (let j = i + 1; j < usedImages.length; j++) {
        if (usedImages[j].image_url !== usedImages[i].image_url) {
  ...
  return coords.map((c, i) => ({
    ...c,
    image_url: usedImages[i].image_url,
            ...
            {items.map((it, i) => (
              ...
              >
                ...
                >
                 ...
                 {it.image_url && <img src={it.image_url } draggable={false} alt={it.alt} />}
                </div>
              </div>
            ))}
          </div>
